#!/usr/bin/env python3
import sys, struct, hashlib, lzma, os
...
def main():
    ...
    outer=find_fvs(data)
    guided=[]
    # enumerate GUID-defined sections from outer FV FFS files
        ...
        while p+24<=end:
            ...
            while sp+4<=se:
                ...
                if st==0x02 and ss>=0x1c:
                    ...
                    guided.append((sp,ss,g,doff,attrs))
                sp=align(sp+ss,4)
            p=align(p+sz,8)
    lines.append(f'guided-section-count: {len(guided)}')
    allnames=[]
    for i,(sp,ss,g,doff,attrs) in enumerate(guided):
        gs=guid_str_le(g); po=sp+doff; payload=data[po:sp+ss]
        lines += [f'guided-{i}-section-offset: 0x{sp:X}',f'guided-{i}-section-size: 0x{ss:X}',f'guided-{i}-guid: {gs}',f'guided-{i}-data-offset: 0x{doff:X}',f'guided-{i}-attributes: 0x{attrs:04X}',f'guided-{i}-payload-size: 0x{len(payload):X}']
        dec=None; fmt='NONE'
        if g==LZMA_GUID_LE:
        ...
        if dec is None:
            lines.append(f'guided-{i}-decompression-status: UNSUPPORTED_OR_FAILED'); continue
    ...
    lines += ['classification: STOCK_UEFI_GUIDED_PAYLOADS_DECOMPRESSED_AND_ENUMERATED','decision: compressed stock UEFI payloads and inner module names were analyzed host-side only. Presence of a module or variable string does not by itself prove runtime KernelBaseAddr/KernelSize values or authorize any FD base, fastboot boot, flashing, or slot change.']

...
grep -q '^primary-mpidr-affinity: 0x0$' "$TMP/pass.txt"
grep -q '^capture-source: SAME_PRE_SEC_CROSS_CORE_RENDEZVOUS$' "$TMP/pass.txt"
grep -q '^secondary-cpu-return-state: HELD_OR_PARKED_AFTER_READ_ONLY_CAPTURE$' "$TMP/pass.txt"

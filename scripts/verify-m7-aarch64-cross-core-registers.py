...
lines = [
    ...
    f"capture-source: {field(raw, 'capture-source') or 'MISSING'}",
    f"secondary-cpu-return-state: {field(raw, 'secondary-cpu-return-state') or 'MISSING'}",
    ...
    f"primary-mpidr-affinity: 0x{primary_affinity:X}" if primary_affinity is not None else "primary-mpidr-affinity: UNAVAILABLE",

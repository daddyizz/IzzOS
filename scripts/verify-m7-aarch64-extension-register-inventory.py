...
checks = [
    ...
    ("baseline-counter-frequency-is-valid", hex_value(baseline, "entry-cntfrq-el0") not in (None, 0)),
    ("baseline-virtual-counter-offset-is-zero", hex_value(baseline, "entry-cntvoff-el2") == 0),
...
lines = [
    ...
    *[f"{label}: {field(inventory, label) or 'MISSING'}" for label in EL2_CONTROL_REGISTERS],
    f"entry-cntfrq-el0: {field(baseline, 'entry-cntfrq-el0') or 'MISSING'}",
    f"entry-cntvoff-el2: {field(baseline, 'entry-cntvoff-el2') or 'MISSING'}",

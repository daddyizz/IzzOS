...
FORBIDDEN_REGEX='gBS->ExitBootServices|gRT->SetVariable|gEfiBlockIoProtocolGuid|gEfiDiskIoProtocolGuid|->WriteBlocks[[:space:]]*\(|->WriteDisk[[:space:]]*\(|MmioWrite(8|16|32|64)[[:space:]]*\(|IoWrite(8|16|32|64)[[:space:]]*\('

if grep -RInE --include='*.c' --include='*.h' "${FORBIDDEN_REGEX}" "${TARGET_DIR}"; then
  echo
...
echo "M1 safety gate passed: no forbidden write/destructive API patterns detected."
